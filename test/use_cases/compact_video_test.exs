defmodule VideoCompactor.UseCases.CompactVideoTest do
  ...
  describe "run/5" do
    test "should compact video", %{repository: repository, video_manager: video_manager, s3_client: s3_client, video_splitter: video_splitter} do
      video = %Video{
      ...
      s3_client |> stub(:download_file, fn _, _ ->
        true = File.exists?("./test/test_utils/test_video.mp4")
        System.cmd("cp", ["-r", "./test/test_utils/test_video.mp4", "/tmp/123e4567-e89b-12d3-a456-426614174000.mp4"])
        assert File.exists?("/tmp/123e4567-e89b-12d3-a456-426614174000.mp4")

        :ok
      end)

      video_splitter |> stub(:split_video, fn _, _ ->
        System.cmd("cp", ["-r", "./test/test_utils/123e4567-e89b-12d3-a456-426614174000/.", "./tmp/123e4567-e89b-12d3-a456-426614174000/"])
    ...
    test "should return error when video_splitter returns error", %{video: video, repository: repository, video_manager: video_manager, s3_client: s3_client, video_splitter: video_splitter} do
      ...
      assert {:error, {:error, "split error"}} = CompactVideo.run(video, repository, video_manager, s3_client, video_splitter)
    end

    test "should return error when repository.create returns error", %{video: video, repository: repository, video_manager: video_manager, s3_client: s3_client, video_splitter: video_splitter} do
      video = %Video{
        id: "123e4567-e89b-12d3-a456-426614174000",
        temp_file_path: "video/123e4567-e89b-12d3-a456-426614174000.mp4",
        extension: "mp4"
      ...
      assert {:error, {:error, "repo error"}} = CompactVideo.run(video, repository, video_manager, s3_client, video_splitter)
    end

    test "should return error when video_manager returns error", %{video: video, repository: repository, video_manager: video_manager, s3_client: s3_client, video_splitter: video_splitter} do
      video = %Video{
        id: "123e4567-e89b-12d3-a456-426614174000",
        temp_file_path: "video/123e4567-e89b-12d3-a456-426614174000.mp4",
        extension: "mp4"
      ...
      assert {:error, {:error, "manager error"}} = CompactVideo.run(video, repository, video_manager, s3_client, video_splitter)
    end
  end
end
